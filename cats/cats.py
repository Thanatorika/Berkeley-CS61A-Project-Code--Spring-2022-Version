...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selected_paragraphs = [x for x in paragraphs if select(x)]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def topic_match(paragraph):
        paragraph_words = remove_punctuation(paragraph).lower().split()
        for topic_word in topic:
            if topic_word in paragraph_words:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if (not typed_words) and (not reference_words):
        ...
        correct_words = 0
        common_length = min(len(typed_words),len(reference_words))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        dic = {word: diff_function(typed_word, word, limit) for word in word_list}
        low_diff = min(dic.values())
        if low_diff <= limit:
            corrected_word = [word for word, diff in dic.items() if diff == low_diff]
...
def sphinx_swaps(start, goal, limit):
    ...
    def substitute(start, goal, limit):
        if start == '' or goal == '':
            return 0
        elif limit == -1:
            return 0
        elif start[0] == goal[0]:
            return substitute(start[1:], goal[1:], limit)
        else:
            return 1 + substitute(start[1:], goal[1:], limit - 1)

    return substitute(start, goal, limit) + abs(len(goal) - len(start))
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit == -1:
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        try_add = minimum_mewtations(start, goal[1:], limit - 1)
        try_remove = minimum_mewtations(start[1:], goal, limit - 1)
        try_substitute = minimum_mewtations(start[1:], goal[1:], limit - 1)

        return 1 + min(try_add, try_remove, try_substitute)
        # END
...
def time_per_word(words, times_per_player):
    ...
    "*** YOUR CODE HERE ***"
    matrix_intervals = []
    for list_of_times in times_per_player:
        list_of_intervals = []
        for index in range(len(list_of_times) - 1):
            list_of_intervals += [list_of_times[index + 1] - list_of_times[index]]
            
        matrix_intervals.append(list_of_intervals)

    result = match(words, matrix_intervals)
...
def fastest_words(match):
    ...
    records = [0] * len(match["words"])
    for w in word_indices:
        min_time = time(match, 0, w)
        fastest_player = 0
        for p in player_indices:
            if time(match, p, w) < min_time:
                min_time = time(match, p, w)
                fastest_player = p
        records[w] = fastest_player
    
    display_records = [0] * len(match["times"])
    for p in player_indices:
        player_records = []
        for w in word_indices:
            if records[w] == p:
                player_records.append(word_at(match, w))
        display_records[p] = player_records
