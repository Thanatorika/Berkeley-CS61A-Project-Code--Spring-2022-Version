...
def roll_dice(num_rolls, dice=six_sided):
    ...
    i, sum_outcome, detect_one = 1, 0, False
    while i <= num_rolls:
        outcome = dice()
        ...
        sum_outcome += outcome
        i += 1
    return sum_outcome if detect_one == False else 1
...
def hefty_hogs(player_score, opponent_score):
    ...
    remnant = opponent_score // 10
    fn = digit_fn(digit)
    result = fn(player_score)
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_dice(*args):
        ...
        averaged_value = total / total_samples
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if score % 10 == opponent_score % 10 != 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
