...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert self.can_contain(other) or other.can_contain(self), 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        return self.find_bee(self.place, 0)

    def find_bee(self, place, dist):
            if place.is_hive:
                return None
            elif place.bees and self.min_range <= dist <= self.max_range:
                return random_bee(place.bees)
            else:
                return self.find_bee(place.entrance, dist + 1)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health > amount:
            ...
            for bee in lst_bees:
                bee.reduce_health(amount)
            lst2_bees = list(place.bees)
            for bee in lst2_bees:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if cls.queen_existence:
            return None
        else:
            cls.queen_existence = True
    ...
    def action(self, gamestate):
        ...
        while curr_place:
            if curr_place.ant:
                if not curr_place.ant.doubled:
                    curr_place.ant.damage *= 2
                    curr_place.ant.doubled = True
                if curr_place.ant.is_container and curr_place.ant.ant_contained and not curr_place.ant.ant_contained.doubled:
                    curr_place.ant.ant_contained.damage *= 2
                    curr_place.ant.ant_contained.doubled = True
